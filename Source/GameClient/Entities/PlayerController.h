...
public:
    ...
    [[nodiscard]] std::uint32_t steamAccountId() const noexcept
    {
        if (!playerControllerPointer)
            return 0;

        const auto steamId = *reinterpret_cast<const std::uint64_t*>(reinterpret_cast<const std::byte*>(playerControllerPointer) + cs2::econ_offsets::kSteamId);
        return static_cast<std::uint32_t>(steamId & 0xFFFFFFFF);
    }
