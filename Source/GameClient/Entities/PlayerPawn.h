...
public:
    ...
    [[nodiscard]] std::optional<cs2::CEntityIndex> crosshairEntityIndex() const noexcept
    {
        if (!playerPawn)
            return {};
        const auto* const indexPtr = reinterpret_cast<const int*>(reinterpret_cast<const std::byte*>(playerPawn) + cs2::combat_offsets::kCrosshairEntityIndex);
        return cs2::CEntityIndex{*indexPtr};
    }
    ...
    [[nodiscard]] Optional<int> shotsFired() const noexcept
    {
        if (!playerPawn)
            return {};
        return *reinterpret_cast<const int*>(reinterpret_cast<const std::byte*>(playerPawn) + cs2::combat_offsets::kShotsFired);
    }

    [[nodiscard]] Optional<cs2::QAngle> aimPunchAngle() const noexcept
    {
        if (!playerPawn)
            return {};
    ...
    [[nodiscard]] Optional<bool> isSpotted() const noexcept
    {
        if (!playerPawn)
            return {};
        return *reinterpret_cast<const bool*>(reinterpret_cast<const std::byte*>(playerPawn) + cs2::combat_offsets::kEntitySpottedState + cs2::combat_offsets::kSpotted);
    }
