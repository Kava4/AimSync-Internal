...
(function () {
  $('#JsSettings').FindChildInLayoutFile('AimSyncMenuTab').SetParent($('#JsMainMenuContent'));

  var openMenuButton = $.CreatePanel('RadioButton', $.GetContextPanel().FindChildTraverse('MainMenuNavBarSettings').GetParent(), 'AimSyncOpenMenuButton', {
    class: "mainmenu-top-navbar__radio-iconbtn",
    group: "NavBar",
    onactivate: "MainMenu.NavigateToTab('AimSyncMenuTab', '');"
  });
  ...
  openMenuButton.SetPanelEvent('onmouseover', function () { UiToolkitAPI.ShowTextTooltip('AimSyncOpenMenuButton', 'AimSync Internal CS2'); });
  openMenuButton.SetPanelEvent('onmouseout', function () { UiToolkitAPI.HideTextTooltip(); });

  $.DispatchEvent('Activated', $.GetContextPanel().FindChildTraverse("MainMenuNavBarHome"), 'mouse');
})();
)");

        if (const auto guiButtonPanel = mainMenu.findChildInLayoutFile("AimSyncOpenMenuButton"))
            state().guiButtonHandle = guiButtonPanel.getHandle();

        if (const auto guiPanel = mainMenu.findChildInLayoutFile("AimSyncMenuTab")) {
    ...
    void onUnload() const noexcept
    {
        ...
        if (auto&& settingsPanel = uiEngine().getPanelFromHandle(state().settingsPanelHandle))
            uiEngine().runScript(settingsPanel, "delete $.AimSync");
    }
