...
class AimSyncDirectoryPath {
public:
    AimSyncDirectoryPath() noexcept
    {
#if IS_WIN64()
        ...
        pathString = mem::makeUniqueForOverwrite<wchar_t[]>(ntPathPrefix.length() + appDataPath.length() + kPathSeparatorLength + build::kAimSyncDirectoryName.length() + kNullTerminatorLength);
        if (!pathString)
            return;
        ...
        std::ranges::copy(build::kAimSyncDirectoryName, pathString.get() + writeIndex);
        writeIndex += build::kAimSyncDirectoryName.length();
        ...
        constexpr auto kPathSeparatorLength{1};
        constexpr auto kNullTerminatorLength{1};
        pathString = mem::makeUniqueForOverwrite<char[]>(homePath.length() + kPathSeparatorLength + build::kAimSyncDirectoryName.length() + kNullTerminatorLength);
        if (!pathString)
            return;
